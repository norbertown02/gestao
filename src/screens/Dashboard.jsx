...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [salesResult, docsResult, goalsResult, portfolioResult] = await Promise.all([
        ...
        supabaseAdmin.from('fiscal_documents').select('issue_date,document_total,movement_type,seller_id,ultra_salesman_id,salesman_name,fiscal_document_items(quantity)').gte('issue_date', iso(range.previousStart)).lte('issue_date', iso(range.end)),
  ...
  const data = useMemo(() => {
    ...
    const quantity = currentDocs.reduce((sum, doc) => sum + (doc.fiscal_document_items || []).reduce((itemSum, item) => itemSum + Number(item.quantity || 0) * Math.sign(fiscalDocumentValue(doc)), 0), 0)
    const previousQuantity = previousDocs.reduce((sum, doc) => sum + (doc.fiscal_document_items || []).reduce((itemSum, item) => itemSum + Number(item.quantity || 0) * Math.sign(fiscalDocumentValue(doc)), 0), 0)
    const series = []
    for (let cursor = new Date(range.start); cursor <= range.end; cursor.setMonth(cursor.getMonth() + 1)) {
      ...
      series.push({
        ...
        Pedidos: currentSales.filter(row => row.sale_date?.startsWith(key)).reduce((sum, row) => sum + netOrderValue(row), 0),
        Faturamento: currentDocs.filter(row => row.issue_date?.startsWith(key)).reduce((sum, row) => sum + fiscalDocumentValue(row), 0),
    ...
    return {
      ...
      salesAttainment: goal ? orderValue / goal * 100 : 0,
  ...
  return <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
    ...
    <div className="page macro-page" style={{ overflowY: 'auto' }}>
      ...
      {loading ? <div className="empty">Preparando visão executiva...</div> : <>
        ...
        <section className="macro-metrics macro-metrics-primary"><Metric icon={IconReceipt} label="Vendas geradas" value={shortMoney(data.orderValue)} current={data.orderValue} previous={data.previousOrderValue} tone="featured" /><Metric icon={IconTargetArrow} label="Meta acumulada" value={data.goal ? shortMoney(data.goal) : '—'} note={data.goal ? `${data.salesAttainment.toFixed(1)}% vendido` : 'meta ainda não cadastrada'} tone="goal" /><Metric icon={IconWallet} label="Saldo para a meta" value={data.goal ? shortMoney(data.goalGap) : '—'} note={data.goal ? (data.goalGap ? 'vendas necessárias' : 'meta de vendas superada') : 'depende da meta cadastrada'} tone={data.goalGap ? 'attention' : 'goal'} /><Metric icon={IconPackage} label="Carteira em aberto" value={shortMoney(data.openValue)} note="posição atual a faturar" tone="portfolio" /></section>
