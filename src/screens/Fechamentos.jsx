...
function CFOFinancialSlides({ data, commercial, period, previousLabel, generatedAt, comparisonNoun, editor }) {
  ...
  return [
    ...
    <IntegratedExecutiveSlide key="integrated" page={15} total={total} commercial={commercial.current} financial={cur} period={period} editor={editor}/>,
  ]
}
...
function aggregateCommercialPeriod(bounds, range, ctx) {
  ...
  ctx.orders.filter(row => row.sale_date && row.sale_date <= endIso && hasNetOrderValue(row)).forEach(row => {
    const key = row.farm_id || row.customer_name
    if (!key) return
