...
function CFOFinancialSlides({ data, period, previousLabel, generatedAt, comparisonNoun, editor }) {
  ...
  const increases = [...costRows].filter(item => item.change > 0).sort((a, b) => b.change - a.change).slice(0, 4)
  const reductions = [...costRows].filter(item => item.change < 0).sort((a, b) => a.change - b.change).slice(0, 4)
  ...
  return [
    ...
    <Slide key="dre" page={3} total={total}>{title('dre','DEMONSTRAÇÃO DO RESULTADO','A receita se converteu em resultado com esta estrutura.','regime de competência')}<div className="deck-dre-flow">{[['Receita',cur.revenue,100,prev.revenue],['Custos variáveis',-Math.abs(cur.variableCosts),cur.variableCostPct,-Math.abs(prev.variableCosts)],['Margem de contribuição',cur.contribution,cur.contributionMargin,prev.contribution],['Custos fixos',-Math.abs(cur.fixedCosts),cur.fixedCostPct,-Math.abs(prev.fixedCosts)],['Resultado operacional',cur.operatingResult,cur.revenue ? cur.operatingResult/cur.revenue*100 : 0,prev.operatingResult],['Resultado líquido',cur.result,cur.netMargin,prev.result]].map(([label,value,share,previous],i)=><div key={label} className={i===5 ? (value>=0?'result positive':'result negative'):''}><span>{label}</span><i><em style={{width:`${Math.min(Math.abs(share),100)}%`}}/></i><strong>{shortMoney(value)}</strong><small>{previousHasData ? `${comparisonNoun}: ${shortMoney(previous)}` : pct(share)}</small></div>)}</div></Slide>,
