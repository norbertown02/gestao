...
function CFOFinancialSlides({ data, period, previousLabel, generatedAt, comparisonNoun, editor }) {
  ...
  const increases = [...accountRows].filter(item => item.change > 0).sort((a, b) => b.change - a.change).slice(0, 4)
  const reductions = [...accountRows].filter(item => item.change < 0).sort((a, b) => a.change - b.change).slice(0, 4)
  ...
  return [
    ...
    <Slide key="dre" page={3} total={total}>{title('dre','DEMONSTRAÇÃO DO RESULTADO','A receita se converteu em resultado com esta estrutura.','regime de competência')}<div className="deck-dre-flow">{[['Receita',cur.revenue,100,prev.revenue,'gain'],['Custos variáveis',-Math.abs(cur.variableCosts),cur.variableCostPct,-Math.abs(prev.variableCosts),'cost'],['Margem de contribuição',cur.contribution,cur.contributionMargin,prev.contribution,'gain'],['Custos fixos',-Math.abs(cur.fixedCosts),cur.fixedCostPct,-Math.abs(prev.fixedCosts),'cost'],['Resultado operacional',cur.operatingResult,cur.revenue ? cur.operatingResult/cur.revenue*100 : 0,prev.operatingResult,'gain'],['Resultado líquido',cur.result,cur.netMargin,prev.result,'gain']].map(([label,value,share,previous,nature],i)=>{const improved=nature==='cost'?Math.abs(value)<=Math.abs(previous):value>=previous;return <div key={label} className={i===5?(value>=0?'result positive':'result negative'):''}><span>{label}</span><i><em style={{width:`${Math.min(Math.abs(share),100)}%`}}/></i><strong>{shortMoney(value)}</strong><small className={previousHasData?(improved?'positive':'negative'):''}>{previousHasData?`${improved?'▼':'▲'} ${comparisonNoun}: ${shortMoney(previous)}`:pct(share)}</small></div>})}</div></Slide>,
    ...
    <Slide key="cost-change" page={7} total={total}>{title('cost_change','VARIAÇÕES DE CUSTO','Quais contas mais pressionaram e quais aliviaram o resultado.',`variação absoluta contra ${comparisonNoun}`)}<div className="deck-cost-change"><section><header><span>MAIORES AUMENTOS</span><b>{shortMoney(increases.reduce((s,i)=>s+i.change,0))}</b></header>{increases.map(item=><div key={item.name}><span>{item.name}</span><i><em style={{width:`${item.change/Math.max(increases[0]?.change||1,1)*100}%`}}/></i><strong>+ {shortMoney(item.change)}</strong></div>)}</section><section><header><span>MAIORES REDUÇÕES</span><b>{shortMoney(Math.abs(reductions.reduce((s,i)=>s+i.change,0)))}</b></header>{reductions.map(item=><div key={item.name}><span>{item.name}</span><i><em style={{width:`${Math.abs(item.change)/Math.max(Math.abs(reductions[0]?.change||1),1)*100}%`}}/></i><strong>− {shortMoney(Math.abs(item.change))}</strong></div>)}</section></div></Slide>,
