...
export default function Estoque() {
  ...
  const data = useMemo(() => {
    ...
    return {
      rows: filtered.sort((a, b) => b.costValue - a.costValue), stock, costValue, billingCapacity, potentialMargin, withoutStock, slow,
    ...
    <div className="page stock-page" style={{ overflowY: 'auto' }}>
      ...
      {loading ? <div className="empty">Carregando estoque do Ultra...</div> : <>
        ...
        <section className="stock-grid">
          <div className="stock-card"><div className="stock-card-head"><div><span>Capital imobilizado</span><h3>Produtos com maior valor em estoque</h3></div></div><ResponsiveContainer width="100%" height={330}><BarChart data={data.topValue} layout="vertical" margin={{ left: 18, right: 26 }}><CartesianGrid strokeDasharray="4 6" horizontal={false} /><XAxis type="number" tickFormatter={value => `${Math.round(value / 1000)}k`} /><YAxis type="category" dataKey="name" width={150} tick={{ fontSize: 10 }} /><Tooltip formatter={value => money(value)} /><Bar dataKey="costValue" name="Valor pelo custo" fill="#486b5a" radius={[0, 6, 6, 0]} /></BarChart></ResponsiveContainer></div>
