...
function saleItemValue(item) {
  ...
  if (Number.isFinite(calculated) && calculated !== 0) return Math.abs(calculated)
  const direct = Number(item?.subtotal ?? item?.total ?? item?.value)
  return Number.isFinite(direct) ? Math.abs(direct) : 0
}
...
export default function VendasFiltradasV2() {
  ...
  function productMaps(list) {
    return {
      ...
      byName: new Map(list.filter(p => p.name).map(p => [normalizedName(p.name), p])),
    }
  }
  ...
  function productFromItem(item, maps) {
    ...
    const name = normalizedName(item?.productName || item?.product_name || item?.product_name || item?.name)
    if (name && maps.byName.has(name)) return maps.byName.get(name)
    if (name.endsWith(' PO') && maps.byName.has(name.replace(/ PO$/, ' PO'))) return maps.byName.get(name.replace(/ PO$/, ' PO'))
    return null
  }
