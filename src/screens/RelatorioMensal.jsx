...
export default function RelatorioMensal() {
  ...
  async function gerarPDF() {
    ...
    // ── GRÁFICO: Cotações vs Vendas ───────────────────────────────────────────
    if(dados.evolucaoMensal && dados.evolucaoMensal.length > 0) {
      checkPage(80)
      titulo('Cotações Geradas vs Vendas Realizadas — últimos 6 meses',13,laranja)
      linha()
      const evol = dados.evolucaoMensal
      const gX = margin, gY = y, gW = W-margin*2, gH = 55
      const maxCot = Math.max(...evol.map(e=>e.cotacoes),1)
      const maxVen = Math.max(...evol.map(e=>e.vendas),1)
...
+vLabel, x+barW*1.5+2, gY+gH-14-hVen-1.5, {align:'center'})
        }
        // Label mês
        doc.setFontSize(6); doc.setTextColor(...cinzaMed); doc.setFont('helvetica','normal')
        doc.text(e.label, x+barW+1, gY+gH-5, {align:'center'})
      })

      // Legenda
      y += gH + 4
      doc.setFillColor(...laranja); doc.rect(gX,y,5,4,'F')
      doc.setFontSize(7); doc.setTextColor(...cinzaEsc); doc.setFont('helvetica','normal')
      doc.text('Cotações geradas (qtd)',gX+7,y+3.5)
      doc.setFillColor(...verde); doc.rect(gX+65,y,5,4,'F')
