...
export default function RelatorioMensalFiltradoMulti(){
  ...
  function filteredSaleAmount(sale,byId,byUltra){const items=parseItems(sale.items);if(!hasNetOrderValue(sale))return{amount:0,qty:0,matched:false};if(!applicationIds.length&&!categoryIds.length&&!product)return{amount:netOrderValue(sale),qty:items.reduce((a,i)=>a+itemQty(i),0),matched:true};let amount=0,qty=0,matched=false;items.forEach(item=>{const p=productForItem(item,byId,byUltra);if(!matches(p))return;amount+=itemValue(item);qty+=itemQty(item);matched=true});return{amount,qty,matched}}
  ...
  async function load(){
    ...
    try{
      ...
      const[sa,sp,visits,visitsPrev,farms,profiles,evol]=await Promise.all([
        supabaseAdmin.from('management_order_overview').select('*').gte('sale_date',ini).lte('sale_date',fim),
        supabaseAdmin.from('management_order_overview').select('*').gte('sale_date',iniAnt).lte('sale_date',fimAnt),
        ...
        supabaseAdmin.from('management_order_overview').select('*').gte('sale_date',iso(d6)).lte('sale_date',fim),
      ])
      const orderRows=[...(sa.data||[]),...(sp.data||[]),...(evol.data||[])];const ids=[...new Set(orderRows.map(r=>r.id).filter(Boolean))];let itemsById=new Map();if(ids.length){const itemRes=await supabaseAdmin.from('sales').select('id,items,comissao_pct').in('id',ids);if(itemRes.error)throw itemRes.error;itemsById=new Map((itemRes.data||[]).map(r=>[String(r.id),r]))}const attach=rows=>(rows||[]).map(r=>{const raw=itemsById.get(String(r.id));return{...r,items:raw?.items||[],comissao_pct:raw?.comissao_pct||0}})
      const currentRows=attach(sa.data),previousRows=attach(sp.data),evolutionRows=attach(evol.data)
      const current=currentRows.map(s=>({...s,...filteredSaleAmount(s,byId,byUltra)})).filter(s=>s.matched)
      const previous=previousRows.map(s=>({...s,...filteredSaleAmount(s,byId,byUltra)})).filter(s=>s.matched)
  ...
  return <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
    <Topbar title="Relatório de Gestão" subtitle="Análise comercial de pedidos de venda por competência e produto" />
    <div className="page relpremium-page" style={{overflowY:'auto'}}>
      <section className="relpremium-hero"><div><span className="relpremium-eyebrow">Relatório executivo Nutrialle</span><h2>Gestão comercial por pedidos de venda</h2><p>Todas as análises deste relatório usam pedidos de venda como base comercial.</p></div><div className="relpremium-period-card"><IconFileText size={28}/><strong>Pedidos de venda</strong><span>Competência selecionada</span></div></section>
